...
import com.android.mymindnotes.presentation.viewmodels.EmotionSortingPopupViewModel
...
class EmotionSortingPopup : AppCompatActivity() {
    ...
    // 뷰모델 객체 주입
    private val viewModel: EmotionSortingPopupViewModel by viewModels()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        emotionGroup.setOnCheckedChangeListener { _: RadioGroup?, checkedId: Int ->
            lifecycleScope.launch {
                viewModel.clickEmotionGroup(checkedId)
            }
        }

        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.emotionGroup.collect {
                    when (it) {
