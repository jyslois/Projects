...
class ChangeNickNameUseCaseTest {
    ...
    @Before
    fun setUp() {
        changeNickNameUseCase = ChangeNickNameUseCase(mockMemberRepository)
    }
    ...
    fun invoke_ReturnsSuccessfulResponse() = runTest(testDispatcher) {
        ...
        coEvery { mockMemberRepository.changeNickName(nickname) } returns flowOf(expectedResponse)

        // When
        val actualResponseFlow = changeNickNameUseCase(nickname)
        ...
        coVerify { mockMemberRepository.changeNickName(nickname) }
    }

    @Test
    fun invoke_ReturnsUnsuccessfulResponse() = runTest(testDispatcher) {
        // Given
        val expectedResponse = ChangeNicknameResponse(code = 3001, msg = "닉네임 변경 실패. 닉네임 양식을 지켜 주세요.")
        coEvery { mockMemberRepository.changeNickName(nickname) } returns flowOf(expectedResponse)
    ...
    fun invoke_ThrowsRuntimeException() = runTest(testDispatcher) {
        // Given
        val errorMsg = "닉네임 변경 실패. 인터넷 연결을 확인해 주세요."
        coEvery { mockMemberRepository.changeNickName(nickname) } throws RuntimeException(errorMsg)
