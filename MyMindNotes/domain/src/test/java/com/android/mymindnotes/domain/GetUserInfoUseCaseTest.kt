...
class GetUserInfoUseCaseTest {
    ...
    @Test
    fun invoke_ReturnsNormalResponse() = runTest(testDispatcher) { // invoke_NormalResponse 함수는 runTest를 사용하여 테스트 코루틴을 동기적으로 실행하고 그 결과 (성공 또는 실패)를 리턴한다
        // Given
        val nickname = "로이스"
    ...
    fun invoke_ThrowsRuntimeException() = runTest {
        // Given
        val errorMessage = "서버와의 통신에 실패했습니다. 인터넷 연결을 확인해 주세요."
        ...
        actualResponseFlow.collect {
            ...
            assertEquals(errorMessage, exception.message) // 예외 메시지가 기대한 메시지와 동일한지 확인
        }
        coVerify { mockMemberRepository.getUserInfo() } // mock 객체가 getUserInfo()를 한 번 호출했는지 확인
    }
}